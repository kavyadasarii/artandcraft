(function(){
  ...
  var blogChips = document.querySelectorAll('.blog-filter-row .chip');
  ...
  // ---- Blog newsletter form (front-end only — wire to your list provider) ----
  var blogNewsletterForm = document.getElementById('blogNewsletterForm');
  var blogNewsletterSuccess = document.getElementById('blogNewsletterSuccess');
  if(blogNewsletterForm){
    blogNewsletterForm.addEventListener('submit', function(e){
      e.preventDefault();
      var valid = Array.prototype.every.call(
        blogNewsletterForm.querySelectorAll('[required]'),
        function(field){ return field.checkValidity(); }
      );
      if(!valid) return;
      blogNewsletterSuccess.classList.add('show');
      blogNewsletterForm.reset();
      setTimeout(function(){ blogNewsletterSuccess.classList.remove('show'); }, 6000);
    });
  }
